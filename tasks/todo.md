...
- [x] main ブランチの CI が全通過していることを確認（CI ✅、CodeQL 実行中）
- [x] git コミット
...
| ファイル | 変更内容 |
| -------- | -------- |
| `.github/dependabot.yml` | `underscore`・`@tootallnate/once` の ignore ルールを追加。`webpack-dev-server` の ignore を `<= 5.2.0` → `>= 0`（全バージョン）に変更。 |

#### 背景

3パッケージは全て `react-scripts@5.0.1` の推移的依存関係に固定されており、
Dependabot が毎回 "No patched version available" で exit 1 を返し続けていた。
ignore ルール追加により繰り返し失敗を防止。根本解決は CRA → Vite 移行。
