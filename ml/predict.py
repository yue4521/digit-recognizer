...
def preprocess_image(image_path):
    ...
    try:
        ...
        print(f"DEBUG: 前処理完了 - 元サイズ: {original_size}, 最終サイズ: {image_array.shape}", file=sys.stderr)
...
def predict_digit(image_path):
    ...
    try:
        ...
        if not os.path.exists(model_path):
            print(f"DEBUG: モデルファイルが見つかりません: {model_path}", file=sys.stderr)
        ...
        print(f"DEBUG: 予測完了 - 結果: {prediction}, 信頼度: {normalized_confidence:.3f}, 処理時間: {processing_time:.3f}秒", file=sys.stderr)
